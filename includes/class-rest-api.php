...
class Rest_Api {
    ...
    public function register_routes(): void {
        ...
        register_rest_route(
            ...
            [
                ...
                'permission_callback' => function ($request) {  // <-- aggiungi $request
                    //error_log('REST API chiamata - Headers: ' . print_r($request->get_headers(), true));
                    //error_log('Nonce ricevuto: ' . $request->get_header('X-Wp-Nonce'));
                    return true; 
                }, 
    ...
    public function handle_register( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // 8a. Azioni post-commit (email, ecc.)
        ...
        // 8b. Invia notifica allo staff
        $this->send_staff_notification(
            $result['registration_id'],
            $campaign
        );
     ...
     */
    private function send_staff_notification( int $registration_id, object $campaign ): void {
        try {
            $email_manager = new Email_Manager();
            $email_manager->send_to_staff( $registration_id, $campaign->template_id );
        } catch ( \Throwable $e ) {
            error_log( "[Alientu Engine] Email staff fallita per iscrizione #{$registration_id}: " . $e->getMessage() );
        }
    }
