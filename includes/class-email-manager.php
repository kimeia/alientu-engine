...
if ( ! $sent ) {
    ...
    /**
     * Invia notifica allo staff per nuova iscrizione.
     *
     * @param int    $registration_id  ID iscrizione.
     * @param string $template_id      ID del template campagna.
     */
    public function send_to_staff( int $registration_id, string $template_id ): void {
        $repo         = new Registration_Repo();
        $registration = $repo->find( $registration_id );
        ...
        // Carica config per email staff
        $config = $this->load_template_config( $template_id );
        $staff_email = $config->emails->staff_email ?? 'staff@alientu.net';

        $subject = $this->get_staff_subject( $config, $registration );
        $body    = $this->render_staff_body( $registration );

        $headers = [
            'Content-Type: text/plain; charset=UTF-8',
            'From: Alientu <noreply@alientu.it>',
            "Reply-To: {$registration->referente_email}",
        ];

        $sent = wp_mail( $staff_email, $subject, $body, $headers );

        if ( ! $sent ) {
            throw new \RuntimeException( "Invio email staff fallito per iscrizione #{$registration_id}." );
        }
    }
     ...
     * Subject email staff.
     */
    private function get_staff_subject( ?object $config, object $registration ): string {
        $type_label = $this->get_type_label( $registration->registration_type );
        $default = "Alientu 2026 — Nuova iscrizione {$type_label} [{$registration->registration_code}]";

        if ( ! $config || ! isset( $config->emails->staff_notification->subject ) ) {
            return $default;
        }

        $subject = $config->emails->staff_notification->subject;
        $subject = str_replace( '{{registration_code}}', $registration->registration_code, $subject );
        $subject = str_replace( '{{type}}', $type_label, $subject );
     ...
     * Body email staff.
     */
    private function render_staff_body( object $registration ): string {
        ...
        $email = $registration->referente_email;
        $phone = $registration->referente_phone ?? 'N/A';
        ...
        $num_players = $payload['_meta']['num_players'] ?? '';

        $body = <<<EOT
NUOVA ISCRIZIONE ALIENTU 2026
════════════════════════════════

Codice: {$code}
Tipo: {$type}
Stato: {$status}
Data: {$registration->created_at}
...
Visualizza in admin: [Link da configurare]
